param(
    [string]$Repo = "ShaggyLorean/slaythespire2-portrait-mobile",
    [string]$ExpectedPackageName = "com.sts2portrait.mobile.dev"
)
...
foreach ($release in $releases) {
    ...
    $assets = @($release.assets | Where-Object { $_.name -match "^StS2Portrait-v.*\.apk$" })
