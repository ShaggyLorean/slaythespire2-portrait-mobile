param(
    ...
    [string]$PackageName = "com.sts2portrait.mobile.local",
    [string]$AndroidHome = "",
    [string]$JavaHome = "",
    [string]$GradlePath = "",
...
if (-not $AndroidHome -or -not (Test-Path -LiteralPath $AndroidHome)) {
    throw "Android SDK not found. Pass -AndroidHome or set ANDROID_HOME/ANDROID_SDK_ROOT."
}
...
if ($PackageName -eq "com.sts2portrait.mobile.dev" -and (Split-Path -Leaf $KeystorePath) -eq "localtest.keystore") {
...
Write-Host "Patching managed assemblies for Android..."
dotnet $patcherDll `
    (Join-Path $bclDir "SteamKit2.dll") `
    (Join-Path $bclDir "STS2Mobile.dll") `
    (Join-Path $bclDir "sts2.dll")
if ($LASTEXITCODE -ne 0) {
...
$apk = Get-ChildItem -LiteralPath (Join-Path $androidDir "build\outputs\apk\mono\release") -Filter "StS2Portrait-v*.apk" |
...
if ($apk.Name -ne "StS2Portrait-v$VersionName.apk") {
    throw "Unexpected APK output after build. Expected StS2Portrait-v$VersionName.apk, got $($apk.Name)"
}
...
$artifactAbiName = $Abi
$archivedApk = Join-Path $artifactDir "StS2Portrait-v$safeVersionName-$artifactAbiName.apk"
