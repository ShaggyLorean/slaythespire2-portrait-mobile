...
internal static class AndroidGodotAppBridge
{
    ...
    internal static long GetDisplayCutoutTopInsetPixels()
        => (long)(GetInstance()?.Call("getDisplayCutoutTopInsetPixels") ?? 0L);

    internal static void SetStatusBarColor(int red, int green, int blue)
        => GetInstance()?.Call("setStatusBarColor", red, green, blue);

    internal static void ClearStatusBarColor()
        => GetInstance()?.Call("clearStatusBarColor");

    internal static void ShowCombatTopCover(int red, int green, int blue)
        => GetInstance()?.Call("showCombatTopCover", red, green, blue);

    internal static void HideCombatTopCover()
        => GetInstance()?.Call("hideCombatTopCover");
