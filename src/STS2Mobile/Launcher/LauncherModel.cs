...
internal partial class LauncherModel : IDisposable
{
    ...
    private readonly OfflineGameImporter _offlineImporter;
    ...
    internal LauncherMode Mode { get; private set; }
    ...
    internal event Action<OfflineGameImporter.ImportProgress> OfflineImportProgressChanged;
    internal event Action OfflineImportCompleted;
    internal event Action<string> OfflineImportFailed;
    ...
    internal LauncherModel(string dataDir)
    {
        ...
        _offlineImporter = new OfflineGameImporter();
        _offlineImporter.ProgressChanged += progress =>
            Raise(OfflineImportProgressChanged, progress, nameof(OfflineImportProgressChanged));
    }

    // Loads saved credentials and determines the launcher path.
    internal FastPathResult StartSession()
    {
        Mode = LauncherMode.SteamOnline;
        _connectionResolved = false;
        return ResolveFastPath();
    }

    internal bool StartOfflineSession()
    {
        Mode = LauncherMode.Offline;
        _connectionResolved = true;
        LauncherCloudSaveState.DisableCloudSyncForLaunch();
        return LauncherGameFiles.Ready(_dataDir);
    }
    ...
    internal void ClearMode()
    {
        ...
        _connectionResolved = true;
        CancelDownload();
        _steamSession.Retry(downloadActive: false);
    }

    internal async Task ImportOfflineAsync()
    {
        Mode = LauncherMode.Offline;
        try
        {
            await _offlineImporter.ImportAsync(_dataDir).ConfigureAwait(false);
            Raise(OfflineImportCompleted, nameof(OfflineImportCompleted));
        }
        catch (Exception ex)
        {
            PatchHelper.Log($"[Offline] Import failed: {ex}");
            Raise(OfflineImportFailed, ex.Message, nameof(OfflineImportFailed));
        }
    }
    ...
    private void PrepareSaveModeForLaunch()
    {
        if (Mode == LauncherMode.Offline)
        {
            LauncherLaunchMarkers.SaveOfflineLaunchMarker();
            LauncherCloudSaveState.DisableCloudSyncForLaunch();
            return;
        }
    ...
    private bool TrySafeAndroidRestart()
    {
        ...
        LauncherLaunchMarkers.SavePendingModeResume(Mode);
