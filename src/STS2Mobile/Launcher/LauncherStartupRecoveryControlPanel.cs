...
internal sealed class LauncherStartupRecoveryControlPanel
{
    ...
    private const int ContainerSeparation = 18;
    private const int DetailFontSize = 29;
    ...
    private static readonly Vector2 ButtonMinimumSize = new(0, 94);
    ...
    private LauncherStartupRecoveryControlPanel(string nodeName)
    {
        ...
        {
            Color = new Color(0.015f, 0.025f, 0.03f, 0.98f),
        ...
        margins.AddThemeConstantOverride("margin_left", 38);
        margins.AddThemeConstantOverride("margin_top", (int)Math.Ceiling(PortraitDisplay.SafeTop() + 32f));
        margins.AddThemeConstantOverride("margin_right", 38);
        margins.AddThemeConstantOverride("margin_bottom", (int)Math.Ceiling(PortraitDisplay.SafeBottom() + 32f));
        Layer.AddChild(margins);
    ...
    private static void AddButton(VBoxContainer box, string label, Action run)
    {
        ...
        button.AddThemeFontSizeOverride(ThemeFontSize, ButtonFontSize);
        button.Pressed += run;
        box.AddChild(button);
    }
