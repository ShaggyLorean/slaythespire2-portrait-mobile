...
internal sealed class LauncherController
{
    ...
    internal void Start()
    {
        ...
        _model.OfflineImportProgressChanged += progress =>
            _runOnMainThread(() => _view.OfflineImport.ShowProgress(progress));
        _model.OfflineImportCompleted += () =>
            _runOnMainThread(CompleteOfflineImport);
        _model.OfflineImportFailed += message =>
            _runOnMainThread(() => FailOfflineImport(message));

        _view.ModeSelection.OfflinePressed += OfflineModePressed;
        _view.ModeSelection.SteamOnlinePressed += SteamOnlineModePressed;
        _view.OfflineImport.ImportPressed += OfflineImportPressed;
        _view.OfflineImport.BackPressed += ChangeModePressed;
        _view.Login.LoginRequested += LoginPressed;
        _view.Login.BackPressed += ChangeModePressed;
        _view.Code.CodeSubmitted += CodeSubmitPressed;
        _view.Code.BackPressed += ChangeModePressed;
        _view.Download.DownloadRequested += DownloadPressed;
        _view.Download.BackPressed += ChangeModePressed;
        ...
        _view.Actions.ChangeModePressed += ChangeModePressed;
        ...
        if (
            _model.InGameMode
            && LauncherLaunchMarkers.TryConsumePendingModeResume(out var resumedMode)
        )
        {
            _resumeLaunchAfterPckRestart = true;
            if (resumedMode == LauncherMode.Offline)
                OfflineModePressed();
            else
                SteamOnlineModePressed();
            return;
        }

        _view.ShowModeSelection();
    }
    ...
    private void OfflineModePressed()
    {
        if (_model.StartOfflineSession())
        {
            ShowReadyToLaunch(
                "Offline files are ready. Steam and Cloud are disabled for this launch.",
                showUpdate: false,
                showCloudSync: false
            );
            ResumeLaunchAfterPckRestart();
            return;
        }

        _view.ShowOfflineImport();
    }

    private void SteamOnlineModePressed()
    {
        LauncherPreferences.LoadAndApplyCloudSyncEnabled();
        _view.HideAllSections();
        var result = _model.StartSession();
        HandleFastPath(result);
        _localLoginHandoffStarted = false;
    ...
    private async void OfflineImportPressed()
    {
        if (!AppPaths.HasStoragePermission())
        {
            AppPaths.RequestStoragePermission();
            _view.SetStatus("Allow file access in Android settings, then try the import again.");
            return;
        }

        _view.SetStatus("Validating and importing local game files...");
        _view.OfflineImport.BeginImport();
        await _model.ImportOfflineAsync();
    }

    private void CompleteOfflineImport()
    {
        ShowReadyToLaunch(
            "Offline import complete. Start the game to finish installing the runtime files.",
            showUpdate: false,
            showCloudSync: false
        );
        _view.AppendLog("Offline import completed from /StS2Portrait/Offline.");
    }

    private void FailOfflineImport(string message)
    {
        _view.OfflineImport.Reset();
        _view.OfflineImport.Visible = true;
        _view.SetStatus($"Offline import failed: {message}");
        _view.AppendLog($"Offline import failed: {message}");
    }
    ...
    private void RedownloadPressed()
    {
        _view.ShowConfirmation(
            ...
            () =>
            {
                ...
                if (_model.Mode == LauncherMode.Offline)
                {
                    _view.ShowOfflineImport();
                    _view.SetStatus("Local files removed. Copy or update the source folder, then import again.");
                    _view.AppendLog("Offline game files were removed for a clean reimport.");
                    return;
                }
    ...
    private void FailUpdateCheckPresentation(Exception ex)
    {
        ...
        _view.Actions.SetUpdateButtonText("Could not check for updates");
    }
    ...
    {
        if (_model.Mode != LauncherMode.SteamOnline)
            return true;
    ...
    private void ShowLoggedIn()
    {
        ...
        {
            ShowReadyToLaunch(
                $"Logged in as {_model.AccountName}",
                showUpdate: true,
                showCloudSync: true
            );
            ResumeLaunchAfterPckRestart();
    ...
    private void ShowReadyToLaunch(
        string status,
        bool showUpdate,
    ...
    private void ShowPreviousLaunchWarningIfNeeded()
    {
        ...
        _view.AppendLog(
            "Open More options, then Troubleshooting, to inspect or export the error."
        );
    ...
    private void ResumeLaunchAfterPckRestart()
    {
        if (!_resumeLaunchAfterPckRestart)
            return;

        _resumeLaunchAfterPckRestart = false;
        PatchHelper.Log("[Launcher] Continuing the launch after game PCK restart");
        _model.Launch();
    }
    ...
    private static class AppUpdateChecker
    {
        private const string LatestReleaseApiUrl =
            "https://api.github.com/repos/ShaggyLorean/slaythespire2-portrait-mobile/releases/latest";
        private const string NameProperty = "name";
        private const string TagNameProperty = "tag_name";
        private const string UserAgent = "StS2-Portrait-Mobile";
        private static readonly TimeSpan Timeout = TimeSpan.FromSeconds(15);

        internal const string RepoReleasesPage =
            "https://github.com/ShaggyLorean/slaythespire2-portrait-mobile/releases/latest";
