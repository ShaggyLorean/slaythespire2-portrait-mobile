...
internal static class LauncherGameStartupRecovery
{
    ...
    )
    {
        ...
        if (recovered)
        {
            ...
            LauncherStartupStatus.Set(
                startupStatus,
                "Main menu recovered after startup stall."
            );
            CompleteSuccessfulStartup(startupStatus);
            return;
        }
    ...
    internal static void MarkStartupObserved(Label startupStatus, Node gameNode)
    {
    ...
    private static void CompleteSuccessfulStartup(Label startupStatus)
    {
        try
        {
            PortraitDisplay.Apply();
            LauncherStartupMarkerTrace.Clear();
            var recoveryControls = startupStatus
                ?.GetParent()
                ?.GetNodeOrNull<CanvasLayer>(LauncherStartupRecoveryControlPanel.NodeName);
            recoveryControls?.QueueFree();
            startupStatus?.QueueFree();
            PatchHelper.Log("Startup UI cleared immediately; scene snapshot retained");
        }
        catch (Exception ex)
        {
            PatchHelper.Log($"Startup UI cleanup failed: {ex.Message}");
        }
    }
}
