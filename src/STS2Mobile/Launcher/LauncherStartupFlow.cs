...
internal static class LauncherStartupFlow
{
    ...
    internal static async Task RunAsync(object game)
    {
        ...
        var offlineLaunch = LauncherLaunchMarkers.ConsumeOfflineLaunchMarker();
        var startupMode = new StartupMode(previousIncompletePhase, manualSafeLaunch, offlineLaunch);
    ...
    private static async Task RunGameStartupAsync(object game, Node gameNode, Label startupStatus)
    {
        try
        {
            ...
            LauncherGameStartupRecovery.MarkStartupObserved(startupStatus, gameNode);
        }
    ...
    private sealed class StartupMode
    {
        ...
        internal StartupMode(
            string previousIncompletePhase,
            bool manualSafeLaunch,
            bool offlineLaunch
        )
        {
            _previousIncompletePhase = previousIncompletePhase;
            ManualSafeLaunch = manualSafeLaunch;
            OfflineLaunch = offlineLaunch;
        }

        private bool ManualSafeLaunch { get; }
        private bool OfflineLaunch { get; }
        ...
        internal string LocalSavesReasonLog
            => OfflineLaunch
                ? "Disabling Steam Cloud injection for the selected Offline launch mode"
                : ManualSafeLaunch
