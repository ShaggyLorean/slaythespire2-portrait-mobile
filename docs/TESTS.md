...
| Short-bucket sweep (1091x1818 canvas, 18:9 phones): full walk green, Skip re-anchored | pc-visual | PASS | `STS2_PCTEST_WINDOW=1200x2000`: 17/17 captures, zero timeouts; hand baseline, campfire row, Neow options and the event band all hold on the short canvas. The loot Skip now anchors under RewardContainerMask (the real content edge) instead of a panel-height guess that drifted into the hand fan when FillScale height-capped the panel |
| Narrow-bucket sweep (984x2164 canvas): full walk + probes, four fixes | pc-visual | PASS | Overnight run at `STS2_PCTEST_WINDOW=1082x2380` (the 1080-mode of the reference device). Full walk, potions, act 3/elite and compendium probes all green. Fixes landed from the sweep: the shared scrim now reaches the art band and event prose lifts above it (its last line used to sit on bare art; then the deeper scrim's dark end covered the title), the loot Skip sits inside the panel corner (it sank into the hand fan below the panel), the hover-tip lane widened past the potion popup, and the compendium shelf is laid out by hand since containers reset child scale on every sort (the right card and stats chip ran off the narrow canvas). Wide-bucket regression after: 17/17, zero timeouts |
