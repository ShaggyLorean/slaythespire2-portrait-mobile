...
Current posture: validate the new portrait package from a clean install. Upstream launcher evidence is retained in [current Android status](current-android-status.md), but it does not certify this integration.
