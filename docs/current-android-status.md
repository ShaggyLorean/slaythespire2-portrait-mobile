...
> Inherited launcher-overhaul snapshot. This records upstream evidence only; it is not validation evidence for the `StS2 Portrait` package or the new portrait/offline-mode integration. Use `portrait-design-audit.md` for the current device matrix.
